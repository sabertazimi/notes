---
sidebar_position: 10
tags: [AI, Generative AI, LLM]
---

# Large Language Model

## Fundamentals

- [Generative Model](./generative-model.md) - Autoregressive and diffusion architectures
- [Patterns](./patterns.md) - Scaling law, emergent ability, hallucination

## Training

- [Pre-Training](./pre-training.md) - Pre-trained models
- [Fine-Tuning](./fine-tuning.md) - `SFT`, instruction-tuning, LoRA
- [Reinforcement Learning](./rl.md) - `PPO`, `GRPO`, agentic RL

## Inference

- [Reasoning](./reasoning.md) - Chain-of-thought and inference acceleration
- [Evaluation](./evaluation.md) - Accuracy, efficiency, and quality metrics

## Applications

- [Retrieval-Augmented Generation](./rag.md) - Retrieval-augmented generation patterns

## Resources

- [Toolchain](./toolchain.md) - Tools and frameworks
