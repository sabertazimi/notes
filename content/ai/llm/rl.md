...
$$
\begin{aligned}
  D_{\text{KL}}(p\|q) &= \sum\limits_{x} p(x) \log_b\left(\frac{p(x)}{q(x)}\right) \\
  &= \sum\limits_{x} p(x) \left(\log_b p(x) - \log_b q(x)\right) \\
  &= \sum\limits_{x} p(x) \log_b p(x) - \sum\limits_{x} p(x) \log_b q(x)
\end{aligned}
$$
