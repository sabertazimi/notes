...
| 分类   | 快捷键      | 功能描述                                      |
| :----- | :---------- | :-------------------------------------------- |
| 代码块 | `[[` / `]]` | 跳转到上一个 / 下一个函数开头 (第一列的 `{`)  |
|        | `[]` / `][` | 跳转到上一个 / 下一个函数结尾 (第一列的 `}`)  |
|        | `[{` / `]}` | 跳转到当前代码块的起始 `{` 或结束 `}`         |
|        | `[(` / `])` | 跳转到当前括号对的起始 `(` 或结束 `)`         |
| 拼写   | `[s` / `]s` | 跳转到上一个 / 下一个拼写错误                 |
| 诊断   | `[d` / `]d` | 跳转到上一个 / 下一个诊断信息 (LSP) 或宏定义  |
| 搜索   | `[i`        | 显示光标下单词在文件中首次出现的一行          |
| 宏定义 | `[D` / `]D` | 列出当前文件及包含文件中所有匹配的宏定义      |
| 注释   | `[/` / `]/` | 跳转到注释 `/* */` 的开头或结尾               |
| 预处理 | `[#` / `]#` | 跳转到上一个 / 下一个未闭合的 `#if` / `#else` |
